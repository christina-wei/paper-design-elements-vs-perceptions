...
%183 were relationships between individual architecture elements and perceptions. These single architecture element are included in our synthesized frame, 
%CAs using multiple conversation architecture elements were discarded because perceptions are measured at the aggregate level, we cannot attribute the perceptions of agents down to its underlying elements.
%Perceptions impacted by the composite of multiple architecture elements in an agent will be discussed in more details in the Composite Architecture Elements section.
